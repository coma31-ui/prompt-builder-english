...
export default function Page() {
  ...
  const prompt = useMemo(() => {
    const subjectLines = (isPortrait || isBoth)
      ? subjects.map((s,i)=>{
          const bits:string[] = [];
          if (s.description) bits.push(s.description);
          if ((dict as any).gender[s.gender]) bits.push((dict as any).gender[s.gender]);
          if ((dict as any).age[s.age]) bits.push((dict as any).age[s.age]);
          const head = bits.length ? `Subject ${i+1}: ${bits.join(', ')}` : '';
          const extras:string[] = [];
          if (s.clothing) extras.push(`clothing: ${s.clothing}`);
          if (s.accessories) extras.push(`accessories: ${s.accessories}`);
          if (s.emphasis) extras.push(`emphasis: ${s.emphasis}`);
          const tail = extras.length ? ` (${extras.join('; ')})` : '';
          return head ? head + tail + '.' : '';
        }).filter(Boolean)
      : [];

    const sceneLines = (isLandscape || isBoth) ? [
      sceneDesc ? `Scene: ${sceneDesc}.` : '',
      sceneLocation ? `Location: ${sceneLocation}.` : '',
      sceneEmphasis ? `Scene emphasis: ${sceneEmphasis}.` : '',
    ].filter(Boolean) : [];

    const cameraTypeEn = cameraType === '一眼レフカメラ' ? 'DSLR' : 'mirrorless';
    const lensEn = `${(dict as any).lens[lens]} ${(dict as any).aperture[aperture]}`.trim();
    const shutterEn = shutter.replace('秒','s');

    const compLines = [
      composition.length ? `Composition: ${composition.map(c=>(dict as any).composition[c]).filter(Boolean).join(', ')}.` : '',
      `Depth of field: ${(dict as any).depth[depth]}.`,
    ...
    {
      const parts:string[] = [];
      parts.push((dict as any).lightingType[ltType]);
      if (kelvin) parts.push(`${kelvin}K color temperature`);
      parts.push((dict as any).lightDir[ltDir]);
      lightingLines.push(`Lighting: ${parts.filter(Boolean).join(', ')}.`);
      if (ltType === '日中の自然光') lightingLines.push('Bright, refreshing midday sunlight with soft natural shadows.');
    }

    const styleEn = `${(dict as any).style[style]} photography, natural lighting`;
    const focusEn = (dict as any).focus[focusSel];

    const moodEn = style === 'シネマティック' ? 'Mood: natural, refined, cinematic atmosphere.' : '';

    const postEn: string[] = [];
    if (postColor)  postEn.push('natural color grading, cinematic tones');
    if (postSharp)  postEn.push('high detail, crisp focus');
    if (postNoise)  postEn.push('low noise, clean image');
    if (postFinish) postEn.push('magazine editorial quality');
    const postLine = postEn.length ? `Post-processing: ${postEn.join(', ')}.` : '';

    const aspect = deriveAspect(ratioBase, orientation);
    const orientationEn = (dict as any).orientation[orientation] || 'square';

    const negative = negatives.map(n => (dict as any).neg[n]).filter(Boolean);
    const negativeLine = negative.length ? `Avoid: ${negative.join(', ')}.` : '';

    const lines = [
      `Output: ${orientationEn} orientation, aspect ratio ${aspect}.`,
      `Mode: ${(dict as any).mode[mode]}.`,
      ...subjectLines,
      ...sceneLines,
      `Style: ${styleEn}.`,
      `Camera: ${cameraTypeEn}, Lens: ${lensEn}.`,
      `Settings: aperture ${(dict as any).aperture[aperture]}, shutter ${shutterEn}, ISO ${iso}.`,
      ...compLines,
      `Focus: ${focusEn}.`,
      ...lightingLines,
      moodEn,
    ...
    return lines.join('\n');
  }, [mode, subjects, sceneDesc, sceneLocation, sceneEmphasis, cameraType, lens, aperture, shutter, iso, composition, perspective, depth, ltType, ltDir, kelvin, style, postColor, postSharp, postNoise, postFinish, negatives, focusSel, orientation, ratioBase, isLandscape, isPortrait, isBoth]);
  ...
  return (
    <div className="min-h-screen p-8 max-w-5xl mx-auto space-y-6">
      ...
      <Card className="card">
        <CardContent>
          <div className="mb-3">
            ...
            <p className="text-sm text-slate-600/80">向きとアスペクト比を選択（1:1 / 3:4 / 16:9）</p>
          </div>
      ...
      <Card className="card">
        <CardContent className="space-y-3">
          <h3 className="text-lg font-semibold text-slate-800">出力（英語の自然文）</h3>
          <Textarea className="font-mono text-sm h-80" value={prompt} readOnly />
          <div className="flex gap-2">
